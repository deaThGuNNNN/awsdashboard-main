...
import { RefreshCw, Settings, Database, Server, BarChart, Table2 } from "lucide-react"
...
export default function Navbar({ onRefresh, activeTab, onTabChange }: NavbarProps) {
  ...
  const isTableView = activeTab === 'ec2' || activeTab === 'rds';
    ...
    <header className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          <Button
            variant="ghost"
            className={`flex items-center space-x-1 ${isTableView ? 'text-primary' : 'text-gray-700'}`}
